...
namespace CBGames.Player
{
    ...
    public class SyncPlayer : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        [SerializeField] protected bool sendPosRot = true;
        ...
        [HideInInspector] [SerializeField] protected bool _ragdolled = false;
...
/*                #region Shooter Template
        ...
        /// <summary>
        /// This is a way to wait until the bones on the avatar are assigned before attempting
        /// to access them. This is designed to work a little better with UMA characters. Calls
        /// the SetLayer and SetTags functions.
        /// </summary>
        protected virtual IEnumerator SetBoneSettings()
        {
            yield return new WaitUntil(() => animator.avatar != null);
            SetLayer();
...
/*        #region Shooter Template
        ...
        /// this work with UMA characters a little better.
        /// </summary>
        ...
        //    view.RPC("SetBool", RpcTarget.Others, "IsGrounded", true);
        //}
        #endregion
        ...
        /// </summary>
        [System.Obsolete("This has been deperacated in favor of the MP_vLadderAction component.", false)]
        ...
        /// <summary>
        /// This is used to play the equip animation across the network
        /// </summary>
        /// <param name="area"></param>
        /// <param name="item"></param>
        public virtual void OnEquipItem(vEquipArea area, vItem item)
        {
            vItemManager itemManager = GetComponent<vItemManager>();
            var equipPoint = itemManager.equipPoints.Find(ep => ep.equipPointName == area.equipPointName);
            if (equipPoint != null && item != null && equipPoint.equipmentReference.item != item)
            {
                if (item.originalObject)
                {
                    ...
                    if (equipment != null)
                    {
                        if (inventory.playItemAnimation)
                        {
                            GetComponent<PhotonView>().RPC("CrossFade", RpcTarget.Others, item.EnableAnim, 0.25f);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// This is used to play the unequip animation across the network.
        /// </summary>
        /// <param name="area"></param>
        /// <param name="item"></param>
        public virtual void OnUnequipItem(vEquipArea area, vItem item)
        {
            if (!item) return;
            vItemManager itemManager = GetComponent<vItemManager>();
            var equipPoint = itemManager.equipPoints.Find(ep => ep.equipPointName == area.equipPointName &&
            ...
            if (equipPoint != null && item != null)
            {
                if (item.originalObject)
                {
                    if (equipment != null)
                    {
                        if (!inventory.isOpen && itemManager.playItemAnimation && !itemManager.inEquip && equipPoint.equipmentReference.equipedObject.activeInHierarchy)
                        {
                            GetComponent<PhotonView>().RPC("CrossFade", RpcTarget.Others, item.DisableAnim, 0.25f);
                        }
                    }
                }
            }
...
/*            #region Shooter Template
        ...
        #region Animations
        //[PunRPC]
        //void SetBool(string boolName, bool boolValue)
        //{
        ...
        [PunRPC]
        protected virtual void CrossFade(string name, float time)
        {
            if (GetComponent<Animator>())
            {
                GetComponent<Animator>().CrossFade(name, time);
            }
        }
        [PunRPC]
...
/*        #region Zipline Addon
        ...
        protected virtual void FreezeRigidbodyContraints(bool enableFreeze)
        {
            ...
            {
                originalContraints = rb.constraints;
                contraintsSet = true;
            }
            rb.constraints = (enableFreeze == true) ? RigidbodyConstraints.FreezeAll : originalContraints;
        }
        [PunRPC]
        ...
        protected virtual void FixedUpdate()
        {
            ...
            {
                if (tpc.isGrounded == false) _hasJumped = true; //This is a hack to get around an Invector bug with endlessly calling the OnActiveRagdoll unityevent
                if (_ragdolled == true)
                {
            ...
            {
                if (sendPosRot == true && !tpc.ragdolled)
                {
                    float distance = Vector3.Distance(transform.position, realPos);
                    transform.position = Vector3.Lerp(transform.position, realPos, Time.fixedDeltaTime * _positionLerpRate);
                    transform.rotation = Quaternion.Lerp(transform.rotation, realRot, Time.fixedDeltaTime * _rotationLerpRate);
                }
            }
