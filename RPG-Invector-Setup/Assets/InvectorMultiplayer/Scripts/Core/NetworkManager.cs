...
namespace CBGames.Core
{
    ...
    public partial class NetworkManager : MonoBehaviourPunCallbacks
    {
        ...
        {
            if (debugging == true) Debug.Log("Getting random spawn point...");
        ...
        {
            if (debugging == true) Debug.Log("Getting team: "+teamName+" spawn point...");
            GameObject[] spawnPoints = GameObject.FindGameObjectsWithTag(spawnPointsTag);
            List<Transform> availableSpawnPoints = new List<Transform>();
            foreach(GameObject spawnPoint in spawnPoints)
            {
                if (spawnPoint.name.ToLower().Trim() == initalTeamSpawnPointNames[teamName].ToLower().Trim())
                {
            ...
            {
                if (debugging == true) Debug.Log("No available team: " + teamName + " spawn points.");
            ...
            if (_levelIndexToLoad != -1)
            {
                ...
                if (autoSpawnPlayer == true)
                {
                    ...
                    else if (_spawnAtPoint == null)
                    {
                        ...
                        GameObject player = NetworkInstantiatePrefab(playerPrefab.name, spawnPoint.transform.position, spawnPoint.transform.rotation, 0);
                        if (gc)
                        {
                            gc.GetType().GetField("currentController", allBindings).SetValue(gc, player.GetComponent<vThirdPersonController>());
                        }
...
/*            #region InvectorModification
        ...
        {
            _connecting = true;
            object outputRoomName;
            expectedRoomProperties.TryGetValue(RoomProperty.RoomName, out outputRoomName);
            _roomName = (string.IsNullOrEmpty((string)outputRoomName)) ? (string)outputRoomName : "";
            if (PhotonNetwork.IsConnected == false || PhotonNetwork.InLobby == false)
            {
