...
namespace CBGames.UI
{
    ...
    public partial class UICoreLogic : MonoBehaviour
    {
        ...
        {
            if (debugging == true) Debug.Log("Setting My Team Name: "+teamName);
            NetworkManager.networkManager.SetTeamName(teamName);
        ...
        public virtual void CreateSessionWithSavedRoomName(bool useSavedPassword = false)
        {
            ...
            if (PhotonNetwork.InLobby && canCreate == true)
            {
                ...
                if (useSavedPassword == true)
                {
                    if (debugging == true) Debug.Log("Creating room: \"" + _roomName + "\" using saved password: \"" + _roomPassword + "\"");
                ...
                {
                    if (debugging == true) Debug.Log("Creating room: \"" + _roomName + "\" with password: \"" + _roomPassword + "\"");
                    NetworkManager.networkManager.CreateRoom(
                        _roomName,
                        customRoomProperties: new ExitGames.Client.Photon.Hashtable {
                            { RoomProperty.Password, _roomPassword },
