...
namespace CBGames.Editors
{
    public partial class E_ConvertPlayer
    {
        ...
        [MenuItem("CONTEXT/vThirdPersonInput/Replace vThirdPersonInput with MP version")]
        public static void CB_MENU_vThirdPersonInput(MenuCommand command)
        {
            vThirdPersonInput comp = (vThirdPersonInput)command.context;
            List<string> log = new List<string>();
            GameObject target = comp.gameObject;
            CB_COMP_vThirdPersonInput(target, ref log);
        }
        ...
        [MenuItem("CONTEXT/vLadderAction/Replace vLadderAction with MP_vLadderAction")]
        ...
        public static void CB_COMP_vThirdPersonInput(GameObject target, ref List<string> log)
        {
            SHOOTER_HasvShooterManagerComp(target);
            if (CB_shooterEnabled) return;

            if (target.GetComponent<vMeleeCombatInput>() || target.GetComponent<MP_vMeleeCombatInput>()) return;
            if (!target.GetComponent<vThirdPersonInput>() || target.GetComponent<MP_vThirdPersonInput>()) return;
            vThirdPersonInput org = target.GetComponent<vThirdPersonInput>();
            MP_vThirdPersonInput dest = target.AddComponent<MP_vThirdPersonInput>();
            E_Helpers.CopyComponentValues(org, dest);
        ...
        public static void CB_COMP_vItemManager(GameObject target, ref List<string> log)
        {
            ...
            if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vItemManager>().onEquipItem, "OnEquipItem", target.GetComponent<SyncPlayer>()))
            {
                if (log != null) log.Add(" * Adding OnEquipItem Event to vItemManager");
                UnityEventTools.AddPersistentListener(target.GetComponent<vItemManager>().onEquipItem, target.GetComponent<SyncPlayer>().OnEquipItem);
            }
            if (!E_PlayerEvents.HasUnityEvent(target.GetComponent<vItemManager>().onUnequipItem, "OnUnequipItem", target.GetComponent<SyncPlayer>()))
            {
                if (log != null) log.Add(" * Adding OnUnequipItem Event to vItemManager");
                UnityEventTools.AddPersistentListener(target.GetComponent<vItemManager>().onUnequipItem, target.GetComponent<SyncPlayer>().OnUnequipItem);
            }
        ...
        public static void CB_COMP_vLadderAction(GameObject target, ref List<string> log)
        {
            ...
            if (!target.GetComponent<MP_vLadderAction>())
            {
                ...
                MP_vLadderAction newComp = target.AddComponent<MP_vLadderAction>();
        ...
        public static void CORE_GetConvertableComponents(GameObject target, ref Dictionary<string, Component> dict)
        {
            ...
            {
                dict.Add("* Replace vHeadTrack -> MP_HeadTrack", target.GetComponent<vHeadTrack>());
            }
            if (target.GetComponent<vLadderAction>())
            {
                dict.Add("* Replace vLadderAction -> MP_vLadderAction", target.GetComponent<vLadderAction>());
            }
            if (target.GetComponent<vThirdPersonInput>())
            {
                dict.Add("* Replace vThirdPersonInput -> MP_vThirdPersonInput", target.GetComponent<vThirdPersonInput>());
            }
        }
        #endregion
    }
